...
export function TopBar() {
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-border-hairline bg-canvas-surface/80 px-6 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-5">
        <div className="flex items-center gap-1.5 text-2xs text-ink-muted">
          <span aria-hidden className={`h-1.5 w-1.5 rounded-full ${statusConfig[status].dot}`} />
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <button
            onClick={logout}
            className="rounded-sm border border-border-subtle px-2 py-1 text-ink-muted transition-colors hover:border-accent/40 hover:text-ink"
          >
            Sair
          </button>
        </div>
